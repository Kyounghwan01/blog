...
## 2.23
- git 꼬임
  - 마스터 브랜치 새로 따서 이전에 작업한 커밋 추적하여 손머지 실시

## 2.24
- sms layout 재수정
  - ux적으로 고객들이 한눈에 알기 편하게 만드는 것이 중요함
  - 디자인이 나왔더라도, ~~하게 바뀔 것이라 예상된다면 감안하고 코딩하는 것이 중요. (어짜피 내 생각대로 디자인이 바뀌게 되더라)
  - qa올려도 디자인이 많이 바뀌니 최대한 빨리 작업하고 qa 올려서 재수정 시간을 버는 것이 중요

## 2.25
- sm : sms 릴리즈
- 월간 예약 제한
    * 수강권 생성/수정 예약 제한 설정 - 완
    * 수강권 layout에 월간/주간 표기 수정 - 완
    * 수강권 회원에게 발급시 ticket값 아닌 usetTicket값 추적 - 완
    * 일괄예약(프라이빗/그룹 모두)시 그 달 예약 횟수 불러와서 예약 제한 수에 따른 예약 성공 실패 도출
        * 프라이빗 - 완, 그룹 - 완
    * 일반 예약시 그 달 예약 횟수 불러와 예약 성공 실패 도출
        * 프라이빗 - 완 , 그룹 - 완
...
- sm : 릴리즈 이후 다음날 처리 할 일

```md
릴리즈 이후

1. 지라 이슈 제거
2. 지라 이슈 동일한 origin 브랜치 명 제거
   2.1 브랜치가 마스터에 포함됬는지 확인

- pr -> base : master, compare : 브랜치 -> 아무 변동사항 없으면 반영됨 -> 브랜치 제거

3. 남아있는 브랜치 master 머지
   staging을 바꾸겠으면 ->
...
- toy : main header
  - MainLayout 이라는 이름으로 컴포넌트 전체를 감싸 일괄 적용
